...
# -------- Config --------
TX_PATH = os.getenv("TX_PATH", "transactions.csv")
PROFILES_PATH = os.getenv("PROFILES_PATH", "customer_profiles.csv")

app = FastAPI(title="Wondr Conversational Insights API", version="0.1.0")
...
def preprocess(tx: pd.DataFrame, profiles: pd.DataFrame) -> pd.DataFrame:
    df = tx.copy()
    ...
    df = df.merge(
        prof[["cif","customer_name","age_group","income_bracket","region","account_type","risk_profile"]],
        on="cif", how="left"
    )
    return df
...
def parse_daterange(text: str, today=None):
    import pandas as pd
    if today is None:
...
def resolve_customer_id(df: pd.DataFrame, query_text: Optional[str] = None):
    if not query_text:
        return str(df["cif"].iloc[0])
...
def aggregate(df: pd.DataFrame, cif: str, start, end):
    ...
    top_category = None if by_cat.empty else by_cat.index[0]
    top_category_amount = 0.0 if by_cat.empty else float(by_cat.iloc[0])
    return d, float(spent), float(income), float(net), top_category, top_category_amount

def load_data():
    global df, DATA_END
    if not os.path.exists(TX_PATH) or not os.path.exists(PROFILES_PATH):
        raise FileNotFoundError(f"Missing CSVs. TX_PATH={TX_PATH}, PROFILES_PATH={PROFILES_PATH}")
    tx = read_csv_flexible(TX_PATH)
    profiles = read_csv_flexible(PROFILES_PATH)
    df_local = preprocess(tx, profiles)
    DATA_END = df_local["trx_dt"].max().normalize()
    return df_local
...
def customers(limit: int = 50):
    dd = df[["cif","customer_name"]].dropna().drop_duplicates().head(limit)
...
def ask(req: AskRequest):
    ...
    today = df["trx_dt"].max()
    start, end = parse_daterange(req.query, today=today)
    end = min(end, DATA_END)
    cif = resolve_customer_id(df, req.customer or req.query)
    d, spent, income, net, top_cat, top_cat_amt = aggregate(df, cif, start, end)
    tl = req.query.lower()

    # Intent: biggest spending category
    if ("biggest" in tl or "top" in tl or "largest" in tl) and "category" in tl and ("spend" in tl or "spending" in tl):
        ans = f"[{cif}] Biggest spending category from {start.date()} to {end.date()}: {top_cat} ({top_cat_amt:,.0f})."
        return AskResponse(
            answer=ans, cif=cif, start_date=str(start.date()), end_date=str(end.date()),
            spent=float(spent), income=float(income), net_saving=float(net),
            top_category=top_cat, top_category_amount=float(top_cat_amt)
        )

    # Intent: spend on specific category
    ...
    if cat_kw:
        ...
        ans = f"[{cif}] Spend on {cat_kw} from {start.date()} to {end.date()}: {total:,.0f}."
        return AskResponse(
            answer=ans, cif=cif, start_date=str(start.date()), end_date=str(end.date()),
            spent=float(spent), income=float(income), net_saving=float(net),
            top_category=top_cat, top_category_amount=float(top_cat_amt)
    ...
    if "save" in tl or "saving" in tl:
        ans = f"[{cif}] Estimated savings {start.date()} to {end.date()}: {net:,.0f} (income {income:,.0f} - spend {spent:,.0f})."
        return AskResponse(
            answer=ans, cif=cif, start_date=str(start.date()), end_date=str(end.date()),
            spent=float(spent), income=float(income), net_saving=float(net),
            top_category=top_cat, top_category_amount=float(top_cat_amt)
    ...
    # Default summary
    ans = f"[{cif}] Summary {start.date()} to {end.date()}: spent {spent:,.0f}, income {income:,.0f}, net {net:,.0f}."
    return AskResponse(
        answer=ans, cif=cif, start_date=str(start.date()), end_date=str(end.date()),
        spent=float(spent), income=float(income), net_saving=float(net),
        top_category=top_cat, top_category_amount=float(top_cat_amt)
